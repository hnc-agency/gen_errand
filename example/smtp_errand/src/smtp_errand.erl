...
start_link(Host, Hello, Opts) ->
	...
	gen_errand:start_link(?MODULE, {Host, Hello, Opts1}, []).

run(Errand) ->
	run(Errand, infinity).

run(Errand, Timeout) ->
	ok=gen_errand:call(Errand, perform),
	ok=gen_errand:wait(Errand, done, Timeout),
	gen_errand:call(Errand, {retrieve, self()}).
...
sleep_time(N, #data{opts=#{delay:=D, backoff_time:=BT, backoff_exp:=BE, jitter:=J}}) ->
	{ok, gen_errand:cooldown(N, D, BT, BE, J)}.
...
handle_event({call, From}, {retrieve, CP}, done, D=#data{conn=Conn, caps=Caps}) when Conn=/=undefined ->
	...
	gen_errand:reply(From, {ok, Conn, Caps}),
